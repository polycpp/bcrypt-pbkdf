...
- Private Blowfish: upstream `index.js` keeps `Blowfish` as a module-local constructor — it is not present in `module.exports`, which only ships `BLOCKS`, `HASHSIZE`, `hash`, and `pbkdf`. The C++ port mirrors that decision by placing `Blowfish` under `include/polycpp/bcrypt_pbkdf/detail/blowfish.hpp` and excluding it from the Doxygen public surface. The detail header is reachable to any caller who really needs it but is not part of the supported API: the expansion schedule used by `expand0state` / `expandstate` is specific to the OpenBSD bcrypt construction and is not a general-purpose Blowfish cipher (OpenSSL's default provider no longer ships Blowfish in modern releases, and the standard ECB/CBC modes are not exercised here).
