// Clean-room Blowfish implementation specialized for the OpenBSD bcrypt-pbkdf
// expansion schedule. The standard initial P-array and S-boxes are part of the
// Blowfish algorithm definition (Bruce Schneier, 1993) and are reproduced here
// from the algorithm specification; they are identical across every Blowfish
// implementation including OpenBSD `blowfish.c` and the upstream JS
// `bcrypt-pbkdf` port.
...
constexpr std::array<std::uint32_t, 18> kInitialP = {
...
constexpr std::array<std::uint32_t, 256> kInitialS0 = {
...
constexpr std::array<std::uint32_t, 256> kInitialS1 = {
...
constexpr std::array<std::uint32_t, 256> kInitialS2 = {
...
constexpr std::array<std::uint32_t, 256> kInitialS3 = {
...
Blowfish::Blowfish() noexcept : p_(kInitialP) {
    s_[0] = kInitialS0;
    s_[1] = kInitialS1;
    s_[2] = kInitialS2;
    s_[3] = kInitialS3;
...
std::uint32_t Blowfish::streamToWord(const std::uint8_t* data,
                                     std::size_t databytes) noexcept {
    // Mirrors upstream `stream2word`: reads four bytes from `data` starting at
    // the keystream pointer `j_`, wrapping around at `databytes`. The first
    // byte read becomes the most significant byte of the returned word.
    std::uint32_t temp = 0;
    for (int i = 0; i < 4; ++i) {
        if (j_ >= databytes) {
            j_ = 0;
...
void Blowfish::encipher(std::uint32_t* x) noexcept {
    // The F-function in upstream JS indexes the cipher state as bytes via
    // aliased Uint8Array. Matching that means we always read from the
    // little-endian byte view of `x`, regardless of host endianness.
    auto byte_at = [&](std::size_t word_index, std::size_t byte_index_inside_word) noexcept -> std::uint8_t {
        // byte_index_inside_word is 0..3 where 0 is least-significant byte.
        return static_cast<std::uint8_t>((x[word_index] >> (8 * byte_index_inside_word)) & 0xffu);
    };

    auto F = [&](std::size_t word_index) noexcept -> std::uint32_t {
        const std::uint8_t b0 = byte_at(word_index, 0);
        const std::uint8_t b1 = byte_at(word_index, 1);
        const std::uint8_t b2 = byte_at(word_index, 2);
        const std::uint8_t b3 = byte_at(word_index, 3);
        return ((s_[0][b3] + s_[1][b2]) ^ s_[2][b1]) + s_[3][b0];
    };
    ...
    //     x[0] ^= F(S, x8, 4) ^ P[i+1];
    //   }
    //   var t = x[0]; x[0] = x[1] ^ P[17]; x[1] = t;
    //
    // Upstream's `x8` is the byte view of the same two-word `x` buffer, so
    // index 0 is x[0]'s bytes and index 4 is x[1]'s bytes. We pass word index
    // 0 and 1 respectively to F() above.
    x[0] ^= p_[0];
    for (std::size_t i = 1; i < 16; i += 2) {
        x[1] ^= F(0) ^ p_[i];
        x[0] ^= F(1) ^ p_[i + 1];
    }
    const std::uint32_t t = x[0];
    x[0] = x[1] ^ p_[17];
    x[1] = t;
}

void Blowfish::expand0state(const std::uint8_t* key, std::size_t keybytes) noexcept {
    // Upstream:
    //   for (i = 0, BLF_J = 0; i < 18; i++) P[i] ^= stream2word(key, keybytes);
    //   BLF_J = 0;
    //   for (i = 0; i < 18; i += 2) { encipher(d, d8); P[i] = d[0]; P[i+1] = d[1]; }
    //   for (i = 0; i < 4; i++)
    //     for (k = 0; k < 256; k += 2) { encipher(d, d8); S[i][k] = d[0]; S[i][k+1] = d[1]; }
    //
    // Note d is initialized fresh to {0, 0} for expand0state (the "zero" key
    // expansion) but is *not* zeroed between encipher calls — encipher mutates
    // it in place and the next encipher consumes the previous output.
    j_ = 0;
    for (std::size_t i = 0; i < 18; ++i) {
        p_[i] ^= streamToWord(key, keybytes);
    }
...
void Blowfish::expandstate(const std::uint8_t* data, std::size_t databytes,
    ...
    //       d[1] ^= stream2word(data, databytes);
    //       encipher(d);
    //       S[i][k] = d[0]; S[i][k+1] = d[1];
    //     }
    //   BLF_J = 0;
    //
    // `d` starts at {0, 0} on first iteration and accumulates between rounds.
    j_ = 0;
    for (std::size_t i = 0; i < 18; ++i) {
        p_[i] ^= streamToWord(key, keybytes);
    }
    ...
    std::uint32_t d[2] = {0, 0};
    for (std::size_t i = 0; i < 18; i += 2) {
        d[0] ^= streamToWord(data, databytes);
        d[1] ^= streamToWord(data, databytes);
        encipher(d);
        p_[i] = d[0];
        p_[i + 1] = d[1];
    }
...
void Blowfish::enc(std::uint32_t* data, std::size_t blocks) noexcept {
